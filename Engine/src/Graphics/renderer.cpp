...
#ifdef _DEBUG
    LOG_INFO("Initializing renderer with width: {} height: {} depth: {}", width,
             height, depth);
#endif
...
renderer::~renderer() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (m_render_fences[i])
            vkDestroyFence(m_device, m_render_fences[i], nullptr);
        if (m_render_finished_semaphores[i])
            vkDestroySemaphore(m_device, m_render_finished_semaphores[i],
                               nullptr);
    }
    if (m_framebuffer) vkDestroyFramebuffer(m_device, m_framebuffer, nullptr);
    if (m_background_pipeline)
        vkDestroyPipeline(m_device, m_background_pipeline, nullptr);
    ...
    if (m_velocity_buffer)
        vkDestroyBuffer(m_device, m_velocity_buffer, nullptr);
    ...
    if (m_velocity_image_view)
        vkDestroyImageView(m_device, m_velocity_image_view, nullptr);
    if (m_velocity_image) vkDestroyImage(m_device, m_velocity_image, nullptr);
    if (m_velocity_image_memory)
        vkFreeMemory(m_device, m_velocity_image_memory, nullptr);
    ...
    if (m_solid_image_view)
        vkDestroyImageView(m_device, m_solid_image_view, nullptr);
    if (m_solid_image) vkDestroyImage(m_device, m_solid_image, nullptr);
    if (m_solid_image_memory)
        vkFreeMemory(m_device, m_solid_image_memory, nullptr);
    ...
    if (m_vector_vertex_buffer)
        vkDestroyBuffer(m_device, m_vector_vertex_buffer, nullptr);
    if (m_vector_vertex_memory)
        vkFreeMemory(m_device, m_vector_vertex_memory, nullptr);
    if (m_vector_pipeline)
    ...
    if (m_streamline_vertex_buffer)
        vkDestroyBuffer(m_device, m_streamline_vertex_buffer, nullptr);
    if (m_streamline_vertex_memory)
        vkFreeMemory(m_device, m_streamline_vertex_memory, nullptr);
    if (m_streamline_graphics_pipeline)
        vkDestroyPipeline(m_device, m_streamline_graphics_pipeline, nullptr);
    if (m_streamline_compute_pipeline)
        vkDestroyPipeline(m_device, m_streamline_compute_pipeline, nullptr);
    if (m_streamline_compute_layout)
        vkDestroyPipelineLayout(m_device, m_streamline_compute_layout, nullptr);
    if (m_streamline_compute_descriptor_layout)
        vkDestroyDescriptorSetLayout(
            m_device, m_streamline_compute_descriptor_layout, nullptr);
    if (m_streamline_descriptor_pool)
        vkDestroyDescriptorPool(m_device, m_streamline_descriptor_pool,
                                nullptr);
}
...
void renderer::init_vulkan_resources() {
    ...
    size_t velocity_buffer_size =
        m_sim_width * m_sim_height * m_sim_depth * sizeof(float4);
    ...
    externalMemoryBufferInfo.handleTypes =
        VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT;
    bufferInfo.pNext = &externalMemoryBufferInfo;

    check_vk_result(
        vkCreateBuffer(m_device, &bufferInfo, nullptr, &m_velocity_buffer));

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(m_device, m_velocity_buffer,
                                  &memRequirements);
    ...
    allocInfo.memoryTypeIndex = utils::find_memory_type(
        memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    allocInfo.pNext = &exportAllocInfo;

    check_vk_result(
        vkAllocateMemory(m_device, &allocInfo, nullptr, &m_velocity_memory));
    check_vk_result(
        vkBindBufferMemory(m_device, m_velocity_buffer, m_velocity_memory, 0));
    ...
    // 2. Create Input Image (R32F) - REMOVED
    {
        ...
        check_vk_result(
            vkCreateSampler(m_device, &samplerInfo, nullptr, &m_input_sampler));
    }
    ...
    // 2b. Create Velocity Image (R32G32F)
    {
        ...
        imageInfo.format =
            VK_FORMAT_R32G32B32A32_SFLOAT;  // Use float4 for velocity in 3D
        ...
        check_vk_result(
            vkCreateImage(m_device, &imageInfo, nullptr, &m_velocity_image));

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(m_device, m_velocity_image,
                                     &memRequirements);
        ...
        allocInfo.memoryTypeIndex =
            utils::find_memory_type(memRequirements.memoryTypeBits,
                                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        check_vk_result(vkAllocateMemory(m_device, &allocInfo, nullptr,
                                         &m_velocity_image_memory));
        check_vk_result(vkBindImageMemory(m_device, m_velocity_image,
                                          m_velocity_image_memory, 0));
        ...
        check_vk_result(vkCreateImageView(m_device, &viewInfo, nullptr,
                                          &m_velocity_image_view));
    }

    // 2c. Create Solid Interop Buffer & Image (R8_UNORM)
    {
        size_t solid_buffer_size =
            m_sim_width * m_sim_height * m_sim_depth * sizeof(uint8_t);
        VkBufferCreateInfo bufferInfo = {};
        bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        ...
        check_vk_result(
            vkCreateBuffer(m_device, &bufferInfo, nullptr, &m_solid_buffer));

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(m_device, m_solid_buffer,
                                      &memRequirements);
        ...
        check_vk_result(
            vkAllocateMemory(m_device, &allocInfo, nullptr, &m_solid_memory));
        check_vk_result(
            vkBindBufferMemory(m_device, m_solid_buffer, m_solid_memory, 0));
        ...
        check_vk_result(
            vkCreateImage(m_device, &imageInfo, nullptr, &m_solid_image));
        ...
        check_vk_result(vkAllocateMemory(m_device, &allocInfo, nullptr,
                                         &m_solid_image_memory));
        check_vk_result(vkBindImageMemory(m_device, m_solid_image,
                                          m_solid_image_memory, 0));
    ...
    // 2d. Create Camera Uniform Buffer (Dynamic)
    {
        ...
        vkGetPhysicalDeviceProperties(application::get_physical_device(),
                                      &properties);
        ...
        if (minUboAlignment > 0) {
            m_camera_buffer_size =
                (m_camera_buffer_size + minUboAlignment - 1) &
                ~(minUboAlignment - 1);
        }
        ...
        check_vk_result(
            vkCreateBuffer(m_device, &bufferInfo, nullptr, &m_camera_buffer));

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(m_device, m_camera_buffer,
                                      &memRequirements);
        ...
        check_vk_result(
            vkAllocateMemory(m_device, &allocInfo, nullptr, &m_camera_memory));
        check_vk_result(
            vkBindBufferMemory(m_device, m_camera_buffer, m_camera_memory, 0));

        check_vk_result(vkMapMemory(m_device, m_camera_memory, 0,
                                    memRequirements.size, 0,
                                    &m_camera_mapped_memory));
    }
    ...
    // 2e. Create Vector Vertex Buffer
    {
        ...
        float w = 0.02f;
        float h = 0.8f;
        vertices.push_back({{-w, 0, -w}, {0, -1, 0}});
        vertices.push_back({{w, 0, -w}, {0, -1, 0}});
        vertices.push_back({{w, 0, w}, {0, -1, 0}});
        vertices.push_back({{-w, 0, -w}, {0, -1, 0}});
        vertices.push_back({{w, 0, w}, {0, -1, 0}});
        vertices.push_back({{-w, 0, w}, {0, -1, 0}});
        vertices.push_back({{-w, h, -w}, {0, 1, 0}});
        vertices.push_back({{w, h, w}, {0, 1, 0}});
        vertices.push_back({{w, h, -w}, {0, 1, 0}});
        vertices.push_back({{-w, h, -w}, {0, 1, 0}});
        vertices.push_back({{-w, h, w}, {0, 1, 0}});
        ...
        vertices = {{{-0.1f, 0.0f, -0.1f}, {0, -1, 0}},
                    {{0.1f, 0.0f, -0.1f}, {0, -1, 0}},
                    {{0.1f, 0.0f, 0.1f}, {0, -1, 0}},
                    {{-0.1f, 0.0f, -0.1f}, {0, -1, 0}},
                    {{0.1f, 0.0f, 0.1f}, {0, -1, 0}},
                    {{-0.1f, 0.0f, 0.1f}, {0, -1, 0}},
                    {{-0.1f, 0.0f, -0.1f}, {-1, 0.5, -1}},
        ...
        check_vk_result(vkCreateBuffer(m_device, &bufferInfo, nullptr,
                                       &m_vector_vertex_buffer));

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(m_device, m_vector_vertex_buffer,
                                      &memRequirements);
        ...
        check_vk_result(vkAllocateMemory(m_device, &allocInfo, nullptr,
                                         &m_vector_vertex_memory));
        check_vk_result(vkBindBufferMemory(m_device, m_vector_vertex_buffer,
                                           m_vector_vertex_memory, 0));
    ...
    // 2f. Create Streamline Vertex Buffer (Storage Buffer)
    {
        ...
        size_t bufferSize =
            m_streamline_vertex_count * sizeof(float) * 4;  // vec4(pos, speed)
        ...
        check_vk_result(vkCreateBuffer(m_device, &bufferInfo, nullptr,
                                       &m_streamline_vertex_buffer));

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(m_device, m_streamline_vertex_buffer,
                                      &memRequirements);
        ...
        check_vk_result(vkAllocateMemory(m_device, &allocInfo, nullptr,
                                         &m_streamline_vertex_memory));
        check_vk_result(vkBindBufferMemory(m_device, m_streamline_vertex_buffer,
                                           m_streamline_vertex_memory, 0));
    }

    // 3. Create Output Image
    m_output_image =
        std::make_shared<image>(m_width, m_height, image_type::rgba);
    ...
    // 4. Create Depth Image
    {
        ...
        check_vk_result(
            vkCreateImage(m_device, &imageInfo, nullptr, &m_depth_image));
        ...
        check_vk_result(vkBindImageMemory(m_device, m_depth_image,
                                          m_depth_image_memory, 0));
...
void renderer::create_render_pass() {
    ...
    colorAttachment.finalLayout =
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;  // Ready for ImGui
...
void renderer::create_pipeline() {
    ...
    check_vk_result(vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr,
    ...
    check_vk_result(vkCreatePipelineLayout(m_device, &pipelineLayoutInfo,
                                           nullptr, &m_pipeline_layout));

    // --- Common Pipeline State ---
    auto vertShaderModule = create_shader_module_from_file(
        "Engine/src/Graphics/Shaders/quad.vert.spv");
    ...
    {
        auto fragShaderModule = create_shader_module_from_file(
            "Engine/src/Graphics/Shaders/raymarch.frag.spv");
        ...
        colorBlendAttachment.blendEnable =
            VK_FALSE;  // Raymarching shader handles blending internally/outputs
                       // final color
        ...
        check_vk_result(vkCreateGraphicsPipelines(
            m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
            &m_background_pipeline));  // Reusing m_background_pipeline handle
                                       // for now
    ...
    // --- 2. Vector Pipeline ---
    {
        auto vertShaderModuleVec = create_shader_module_from_file(
            "Engine/src/Graphics/Shaders/vector.vert.spv");
        auto fragShaderModuleVec = create_shader_module_from_file(
            "Engine/src/Graphics/Shaders/vector.frag.spv");

        VkPipelineShaderStageCreateInfo vertShaderStageInfoVec = {};
        vertShaderStageInfoVec.sType =
            VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        ...
        VkPipelineShaderStageCreateInfo shaderStagesVec[] = {
            vertShaderStageInfoVec, fragShaderStageInfoVec};
        ...
        check_vk_result(vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1,
                                                  &pipelineInfoVec, nullptr,
                                                  &m_vector_pipeline));
    ...
    {
        auto vertShaderModuleStr = create_shader_module_from_file(
            "Engine/src/Graphics/Shaders/streamline.vert.spv");
        auto fragShaderModuleStr = create_shader_module_from_file(
            "Engine/src/Graphics/Shaders/streamline.frag.spv");

        VkPipelineShaderStageCreateInfo vertShaderStageInfoStr = {};
        vertShaderStageInfoStr.sType =
            VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        ...
        VkPipelineShaderStageCreateInfo shaderStagesStr[] = {
            vertShaderStageInfoStr, fragShaderStageInfoStr};
        ...
        inputAssemblyStr.sType =
            VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
        inputAssemblyStr.topology =
            VK_PRIMITIVE_TOPOLOGY_POINT_LIST;  // or LINE_LIST
        ...
        check_vk_result(vkCreateGraphicsPipelines(
            m_device, VK_NULL_HANDLE, 1, &pipelineInfoStr, nullptr,
            &m_streamline_graphics_pipeline));
    ...
    // --- 4. Streamline Compute Pipeline ---
    {
        ...
        check_vk_result(vkCreateDescriptorSetLayout(
            ...
            &m_streamline_compute_descriptor_layout));
        ...
        check_vk_result(vkCreatePipelineLayout(m_device, &pipelineLayoutInfo,
                                               nullptr,
                                               &m_streamline_compute_layout));

        // Pipeline
        auto compShaderModule = create_shader_module_from_file(
            "Engine/src/Graphics/Shaders/streamline.comp.spv");
        ...
        check_vk_result(
            vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                     nullptr, &m_streamline_compute_pipeline));
        ...
        check_vk_result(vkCreateDescriptorPool(m_device, &poolInfo, nullptr,
                                               &m_streamline_descriptor_pool));

        std::vector<VkDescriptorSetLayout> layouts(
            MAX_FRAMES_IN_FLIGHT, m_streamline_compute_descriptor_layout);
        ...
        check_vk_result(vkAllocateDescriptorSets(
            m_device, &allocInfo, m_streamline_descriptor_sets.data()));
    ...
    check_vk_result(vkAllocateDescriptorSets(m_device, &allocInfo,
...
void renderer::create_framebuffers() {
    ...
    std::array<VkImageView, 2> attachments = {outputView, m_depth_image_view};
    ...
    check_vk_result(vkCreateFramebuffer(m_device, &framebufferInfo, nullptr,
                                        &m_framebuffer));
}
...
void renderer::create_command_buffers() {
    ...
    check_vk_result(
        vkCreateCommandPool(m_device, &poolInfo, nullptr, &m_command_pool));
    ...
    check_vk_result(vkAllocateCommandBuffers(m_device, &allocInfo,
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        check_vk_result(vkCreateSemaphore(m_device, &semaphoreInfo, nullptr,
                                          &m_render_finished_semaphores[i]));
        check_vk_result(
            vkCreateFence(m_device, &fenceInfo, nullptr, &m_render_fences[i]));
    }
}
...
VkShaderModule renderer::create_shader_module_from_file(
...
void renderer::update_sim_data() {
    ...
    check_vk_result(
        vkBeginCommandBuffer(m_transfer_command_buffer, &beginInfo));
    ...
    vkCmdPipelineBarrier(
        m_transfer_command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
    ...
    region.imageExtent = {(uint32_t)m_sim_width, (uint32_t)m_sim_height,
                          (uint32_t)m_sim_depth};

    vkCmdCopyBufferToImage(m_transfer_command_buffer, m_velocity_buffer,
                           m_velocity_image,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

    // Copy Solid
    vkCmdCopyBufferToImage(m_transfer_command_buffer, m_solid_buffer,
                           m_solid_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           1, &region);
...
void renderer::update_camera(float dt) {
    ...
    cam_data.sim_dimensions =
        glm::vec3((float)m_sim_width, (float)m_sim_height, (float)m_sim_depth);

    char* mappedData = (char*)m_camera_mapped_memory;
    memcpy(mappedData + (m_current_frame * m_camera_buffer_size), &cam_data,
           sizeof(device_camera));
}

void renderer::render() {
    vkWaitForFences(m_device, 1, &m_render_fences[m_current_frame], VK_TRUE,
                    UINT64_MAX);
    ...
    vkCmdBeginRenderPass(m_command_buffers[m_current_frame], &renderPassInfo,
                         VK_SUBPASS_CONTENTS_INLINE);

    vkCmdBindPipeline(m_command_buffers[m_current_frame],
                      VK_PIPELINE_BIND_POINT_GRAPHICS,
                      m_background_pipeline);  // Using background pipeline
                                               // handle for raymarching
    ...
    vkCmdBindDescriptorSets(m_command_buffers[m_current_frame],
                            ...
                            0, 1, &m_descriptor_sets[m_current_frame], 1,
                            &dynamicOffset);
    ...
    // Draw Vectors
    vkCmdBindPipeline(m_command_buffers[m_current_frame],
                      VK_PIPELINE_BIND_POINT_GRAPHICS, m_vector_pipeline);
    ...
    vkCmdPushConstants(m_command_buffers[m_current_frame], m_pipeline_layout,
                       VK_SHADER_STAGE_VERTEX_BIT, 0,
                       sizeof(VectorPushConstants), &pushConstants);

    // Instance count = total cells
    uint32_t instanceCount = m_sim_width * m_sim_height * m_sim_depth;
    vkCmdDraw(m_command_buffers[m_current_frame], m_vector_vertex_count,
              instanceCount, 0, 0);
    ...
    vkCmdDraw(m_command_buffers[m_current_frame], m_streamline_vertex_count, 1,
              0, 0);
    ...
    check_vk_result(vkQueueSubmit(graphicsQueue, 1, &submitInfo,
                                  m_render_fences[m_current_frame]));

    // Synchronize with Main App
    application::get().add_wait_semaphore(
        m_render_finished_semaphores[m_current_frame]);
...
void renderer::compute_streamlines() {
    ...
    vkCmdBindPipeline(m_transfer_command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                      m_streamline_compute_pipeline);

    vkCmdBindDescriptorSets(
        m_transfer_command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE,
        m_streamline_compute_layout, 0, 1,
        &m_streamline_descriptor_sets[m_current_frame], 0, nullptr);
    ...
    vkCmdPushConstants(m_transfer_command_buffer, m_streamline_compute_layout,
                       VK_SHADER_STAGE_COMPUTE_BIT, 0,
                       sizeof(StreamlinePushConstants), &pushConstants);

    vkCmdDispatch(m_transfer_command_buffer,
                  (pushConstants.numSeeds + 255) / 256, 1, 1);
    ...
    check_vk_result(vkQueueSubmit(application::get_graphics_queue(), 1,
