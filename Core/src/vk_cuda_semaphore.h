...
class cuda_vulkan_semaphore {
   public:
    cuda_vulkan_semaphore(VkDevice device) : m_device(device) {
        ...
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &m_semaphore) !=
            VK_SUCCESS) {
        ...
        if (vkGetSemaphoreFdKHR(device, &semaphoreGetFdInfo, &fd) !=
            VK_SUCCESS) {
        ...
        if (cudaImportExternalSemaphore(&m_cuda_semaphore,
    ...
    void wait(cudaStream_t stream) {
        ...
        cudaWaitExternalSemaphoresAsync(&m_cuda_semaphore, &waitParams, 1,
                                        stream);
    }
    ...
    void signal(cudaStream_t stream) {
        ...
        cudaSignalExternalSemaphoresAsync(&m_cuda_semaphore, &signalParams, 1,
                                          stream);
    }
